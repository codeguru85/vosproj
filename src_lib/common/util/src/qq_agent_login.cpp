#include "qq_agent_login.h"
#include "cgi_string_encode.h"
...
void MakeQQMsgJumpSignature(
                            CheckSession_T& stCheckSession,
                            char sSignature[], // [IN/OUT]
                            short& shSigLen,   // [IN/OUT]
                            const char* pszKey
                            )
{
    char achDecryptBuf[sizeof(stCheckSession)];
    memset(achDecryptBuf, 0, sizeof(achDecryptBuf));
    int iDecryptBufLen = sizeof(achDecryptBuf);
    
    stCheckSession.dwUin = htonl(stCheckSession.dwUin);
    stCheckSession.dwTimestamp = htonl(stCheckSession.dwTimestamp);
    
    memcpy(achDecryptBuf, &stCheckSession, sizeof(stCheckSession));
    int iSigLen = 0;

    oi_symmetry_encrypt2(
        achDecryptBuf, iDecryptBufLen,
        pszKey, sSignature, &iSigLen);

    shSigLen = iSigLen;
}

char* bin2str(const unsigned char* sBin, char* sAscii, int binlen)
{
...
string GetMsgUrl(const u_int32_t iQQ, const int iType)
{    
    CheckSession_T stCheckSession;
	char           achSigBuf[MAX_DECRYPT_BUF_LEN];
	char           achStrBuf[MAX_DECRYPT_BUF_LEN];
    short          shSigLen = 0;
	char           sUrl[1024];

	memset(&stCheckSession, 0, sizeof(stCheckSession));
    stCheckSession.dwUin = iQQ;
    stCheckSession.dwTimestamp = time(NULL);
    
    memset(achSigBuf, 0, sizeof(achSigBuf));
    
	// Encrpt
	MakeQQMsgJumpSignature(
                     stCheckSession,
                     achSigBuf,// [IN/OUT]
                     shSigLen, // [IN/OUT]
                     QQ_MSG_JUMP_KEY
    ...
    // Bin2Str
	assert(MAX_DECRYPT_BUF_LEN >= shSigLen * 2);
	memset(achStrBuf, 0, sizeof(achStrBuf));
	bin2str((unsigned char*)achSigBuf, achStrBuf, shSigLen);
	snprintf(sUrl,sizeof(sUrl),"http://member.paipai.com/cgi-bin/qq_agent_login?u=%u&p=%s&c=%d",iQQ,achStrBuf, iType);

	return string(sUrl);
}

extern string GetClientMsgUrl(const u_int32_t iQQ, const string &sDestUrl, const string &sSignUrl)
{
    CheckSession_T stCheckSession;
	char           achSigBuf[MAX_DECRYPT_BUF_LEN];
	char           achStrBuf[MAX_DECRYPT_BUF_LEN];
	...
	snprintf(sUrl, sizeof(sUrl), "%s?clientuin=%u&clientkey=%s&url=%s",
		                          sSignUrl.c_str(),
		                          iQQ,
		                          achStrBuf, 
		                          String2URL(sDestUrl).c_str());
