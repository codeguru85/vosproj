...
private function getIsSetVideoInfoHandler(event:ResultEvent):void
{
	...
	if(flag==true){
		...
		videoInfoService.updateVideoInfo(videoInfo,getCheckedVideoTypes(),videoAlbumShip,keyWords);
	}
}
...
private function setVideoTypesTree(videoTypes:ArrayCollection):void{
	for(var i:int=0;i<videoTypes.length;i++){
		parentNodeSelected(videoTypeTree.children(),VideoType(videoTypes.getItemAt(i)));
	}
}
...
private function getVideoInfo():void{
 ...
 private function getVideoInfoHandler(event:ResultEvent):void{
	...
	var videoTypes:ArrayCollection = videoInfo.videoTypes;
