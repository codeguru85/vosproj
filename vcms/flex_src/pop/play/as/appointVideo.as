...
private function getVideoMessage(evet:ListEvent):void{ 
	...
	videoInfoMessage.height = 260 ;
	videoInfoMessage.labVideoDesc.height = 44;
	videoInfoMessage.labVideoName.text = evet.itemRenderer.data.videoName;	
	videoInfoMessage.labFileSize.text = evet.itemRenderer.data.fileSize;
	videoInfoMessage.labTimeCount.text = evet.itemRenderer.data.timeCount; 
	videoInfoMessage.labVideoDesc.text = evet.itemRenderer.data.descn;
	videoInfoMessage.labImage.source = "./"+evet.itemRenderer.data.imagePath;
	videoInfoMessage.labCreateTime.text = fdate.format(evet.itemRenderer.data.createTime);
	...
	if(videoType.length>0){
		...
		videoInfoMessage.labTypeName.text = strTypeName;
	}else{
		videoInfoMessage.labTypeName.text ="无分类";
	}
} 
...
private function getTermMessage(event:ListEvent):void{
	...
	termInfoMessage.labTermName.text = event.itemRenderer.data.termName;
	termInfoMessage.labTermIp.text = event.itemRenderer.data.termIp;
	termInfoMessage.labTermMask.text = event.itemRenderer.data.termMask;
	termInfoMessage.labTermGateway.text = event.itemRenderer.data.termGateway;
	termInfoMessage.labTermDns.text = event.itemRenderer.data.termDns;
	termInfoMessage.labTermAddr.text = event.itemRenderer.data.termAddr;
	if(event.itemRenderer.data.termType)
		termInfoMessage.labType.text = event.itemRenderer.data.termType.typeName;
	if(event.itemRenderer.data.termActivation == 0){
		termInfoMessage.labTermActivation.text = "否";
	}else{
		termInfoMessage.labTermActivation.text = "是";
	}	 
}
...
private function upMoveHandler(event:MouseEvent):void{
//	var dg:DataGrid = new DataGrid();
	var item:*;
	var i : int;
	if(viewVS.selectedChild == videoWindow){
//		dg = horizontallist;
//		dg.selectedIndex = horizontallist.selectedIndex;
		var item:* = horizontallist.selectedItem;
	}		   
	else if(viewVS.selectedChild == albumWindow){
//		dg = horizontallist2;
//		dg.selectedIndex = horizontallist2.selectedIndex;
		dg.selectedItem = horizontallist2.selectedItem;
	}	
//	var i : int = dg.selectedIndex;
	if(item)
		i = moveimages.getItemIndex(item);
 	if (i >= 1 && item)   
    {
//		var dataProvider:IList = IList(dg.dataProvider);
		
    	item.setTime = moveimages.getItemAt(i-1).setTime;
    	
 		moveimages.getItemAt(i-1).setTime = fdate.format(
 							dateAdd("milliseconds",item.timeSecond*1000,
 							isoToDate(item.setTime)));	
	 		               
    	moveimages.addItemAt(item,i-1);                     
    	moveimages.removeItemAt(i+1);   
  		horizontallist.selectedItem = item;
  		horizontallist2.selectedItem = item;
    }
}
private function downMoveHandler(event:MouseEvent):void{
	var dg:DataGrid = new DataGrid();
	if(viewVS.selectedChild == videoWindow){
		dg = horizontallist;
		dg.selectedIndex = horizontallist.selectedIndex;
		dg.selectedItem = horizontallist.selectedItem;
	}		   
	else if(viewVS.selectedChild == albumWindow){
		dg = horizontallist2;
		dg.selectedIndex = horizontallist2.selectedIndex;
	...
	var e : int = dg.selectedIndex;   
	var length:int = ArrayCollection(horizontallist.dataProvider).length;
    if (e < (length - 1) && horizontallist.selectedItem)   
    {
    	var dataProvider:IList = IList(dg.dataProvider);
    	dataProvider.getItemAt(e+1).setTime = dg.selectedItem.setTime;
    	
		dg.selectedItem.setTime = fdate.format(
 						dateAdd("milliseconds",dataProvider.getItemAt(e+1).timeSecond*1000,
						isoToDate(dataProvider.getItemAt(e+1).setTime)));
                    
        dataProvider.addItemAt(horizontallist.selectedItem,e + 2);                     
        dataProvider.removeItemAt(e);      
        dg.selectedIndex = e;
        horizontallist = dg;     
    }
}
...
private function changeHandler():void{
	this.videoinfo.videoName = "";
   	...
   	albumIdTemp = "";
   	typeIdTemp = "";
   	videoInfoMessage.visible = false;
//   	maintainFocus();
