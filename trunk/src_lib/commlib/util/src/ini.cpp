...
unsigned CIniFile::AddKeyName( string keyname )
{
    Trim( keyname );
...
bool CIniFile::SetValue( string keyname, string valuename, string value, bool const create )
{
    Trim( keyname );
    Trim( valuename );
    Trim( value );
...
void CIniFile::Trim( string & ss )
{
    string::iterator p = find_if( ss.begin(), ss.end(), not1( ptr_fun( ::isspace ) ) );
    ss.erase( ss.begin(), p );

    string::reverse_iterator p2 = find_if( ss.rbegin(), ss.rend(), not1( ptr_fun( ::isspace ) ) );
    ss.erase( p2.base(), ss.end() );
}

string CIniFile::Dump()
{
	stringstream ss;
    for ( unsigned keyID = 0; keyID < NumKeys(); ++keyID ) {
        ss << "Key = [" << KeyName( keyID )<<"]" << endl;
        for ( unsigned valueID = 0; valueID < NumValues( keyID ); ++valueID )
            ss << "   ValueName = [" << ValueName( keyID, valueID ) <<
            "]  Value = [" << GetValue( keyID, valueID )<<"]" << endl;
    }
	return ss.str();
}
