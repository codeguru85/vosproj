...
int		QueueOper::PushDataIntoQueueBack(char *sValue,int iValueLen)
{
	int			iPkgLenWord;
...
int		QueueOper::ReadDataFromQueueHead(char *pToStoreSpace,int iSpaceLen)
{
	...
	if (iPkgLenWord <= iSpaceLen)
		...
		memcpy(pToStoreSpace,m_sBuffTmp,iSpaceLen);

	return	iPkgLenWord;
}

int		QueueOper::StoreData(char *sData,int iDataLen)
{
	int			iCopyLen;
...
int		QueueOper::GetInQueuePkgNum()
{
	if (!m_pQueueCtl)
	{
		snprintf(m_sErrMsg,sizeof(m_sErrMsg),"Get queue pkg num fail:Not init locally");
		return	-1;	
	}	
